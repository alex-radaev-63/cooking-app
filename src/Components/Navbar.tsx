...
const Navbar = () => {
  ...
  return (
    <header
      className="flex flex-row z-1000 
      justify-center
      h-[60px]
          ...
          {user ? (
            <div className="block relative" ref={profileRef}>
              <button
                ...
                className="flex h-full pl-6 items-center cursor-pointer gap-2 text-[var(--color-text)]/85 hover:text-[var(--color-text)]/100 transition"
              >
        ...
        <nav
          className={`absolute z-999 inset-x-0 top-[60px]
                ...
                sm:hidden ${
                  open
                    ? "max-h-screen"
                    : "max-h-0 border-0 duration-350 ease-out"
                }`}
        >
          <ul className="flex flex-col gap-1 px-4 py-3 pt-5 text-lg">
            {/* <li>
              <NavLink
                onClick={() => setOpen(false)}
                to="/"
